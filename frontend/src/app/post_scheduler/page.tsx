...
interface Slot { weekday: number; hour_24: number; score: number; }
    ...
    <div className="overflow-x-auto">
      <div className="grid grid-cols-[auto_1fr] gap-3">
        <div className="grid grid-rows-7 gap-1 text-right pr-2">
          {WEEKS.map((d) => ( <div key={d} className="text-xs text-slate-400 h-6 flex items-center justify-end">{d}</div> ))}
        </div>
        <div className="grid grid-rows-7 gap-1">
          {heatmap.map((row, wi) => (
            <div key={wi} className="grid grid-cols-24 gap-1">
              {row.map((v, h) => {
                const isHi = highlight && highlight.weekday === wi && highlight.hour_24 === h;
                const bucket = v >= 0.75 ? 3 : v >= 0.5 ? 2 : v >= 0.25 ? 1 : 0;
                const base = bucket === 0 ? "bg-slate-700/60" : bucket === 1 ? "bg-indigo-800/60" : bucket === 2 ? "bg-indigo-700" : "bg-indigo-500";
...
// Page
// -----------------------------
const TZ_OPTIONS = [ { id: "Asia/Colombo", label: "Asia/Colombo (UTC+5:30)" }, { id: "Asia/Kolkata", label: "Asia/Kolkata (UTC+5:30)" }, { id: "Asia/Singapore", label: "Asia/Singapore (UTC+8)" }, { id: "Europe/London", label: "Europe/London (UTC±0)" }, { id: "America/New_York", label: "America/New_York (UTC-5)" }, { id: "America/Los_Angeles", label: "America/Los_Angeles (UTC-8)" }, ];
...
export default function PostSchedulerPage() {
  ...
  const onSuggest = async (useLLM = true) => {
    ...
    try {
      ...
      setMessage( typeof e === "object" && e !== null && "message" in e ? String((e as { message?: string }).message) : "Failed" );
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSchedule = () => {
    ...
    const item: QueueItem = { id: `${Date.now()}`, platform, timezone, contentPreview: content.length > 80 ? content.slice(0, 77) + "…" : content, scheduledAt: bestISO, status: "scheduled", };
        ...
        <header className="mb-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              ...
              <p className="text-slate-300 mt-2 max-w-2xl">Enter your content and find the optimal time to post based on real-world data.</p>
            </div>
            {message && ( <div className="bg-emerald-500/15 text-emerald-200 border border-emerald-500/30 rounded-xl px-4 py-2 shadow-soft">{message}</div> )}
          </div>
        </header>
          ...
          <div className="bg-slate-800/60 rounded-2xl p-6 shadow-soft backdrop-blur-sm border border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
              ...
              <div>
                ...
                <select className="w-full rounded-xl bg-slate-900/60 border border-white/10 px-3 py-2" value={timezone} onChange={(e) => setTimezone(e.target.value)}>
                  {TZ_OPTIONS.map((tz) => ( <option key={tz.id} value={tz.id}>{tz.label}</option> ))}
                </select>
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1">Content Type</label>
                <select className="w-full rounded-xl bg-slate-900/60 border border-white/10 px-3 py-2" value={contentType} onChange={(e) => setContentType(e.target.value)}>
                  {CONTENT_TYPES.map((t) => ( <option key={t}>{t}</option> ))}
                </select>
              </div>
            </div>
            <div className="mb-4">
              <label className="block text-sm text-slate-300 mb-1">Post Content</label>
              <textarea className="w-full min-h-[120px] rounded-xl bg-slate-900/60 border border-white/10 px-3 py-2" placeholder="Paste your content here..." value={content} onChange={(e) => setContent(e.target.value)} />
            </div>
            <div className="flex flex-wrap gap-3">
              <button onClick={() => onSuggest(true)} disabled={loading} className="px-4 py-2 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:opacity-95 disabled:opacity-50">
            ...
            {bestISO && reason && (
              <div className="mt-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
                ...
                <div className="mt-3 pt-3 border-t border-emerald-500/20">
                  <h4 className="font-semibold text-emerald-200 text-sm mb-2">
                    {reason.headline}
                  </h4>
          ...
          <div className="space-y-6">
            <div className="bg-slate-800/60 rounded-2xl p-6 shadow-soft border border-white/5">
              ...
              {heatmap ? (
                ...
                <div className="text-slate-400 text-sm">No data yet. Click Suggest to load heatmap.</div>
              )}
            </div>
            <div className="bg-slate-800/60 rounded-2xl p-6 shadow-soft border border-white/5">
              <h2 className="text-xl font-semibold mb-4">Scheduled Queue</h2>
              {queue.length === 0 ? ( <div className="text-slate-400 text-sm">No items yet.</div> ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="text-slate-300">
                      <tr className="text-left border-b border-white/10">
                        <th className="py-2 pr-3">When</th> <th className="py-2 pr-3">Platform</th> <th className="py-2 pr-3">Content</th> <th className="py-2 pr-3">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {queue.map((q) => (
